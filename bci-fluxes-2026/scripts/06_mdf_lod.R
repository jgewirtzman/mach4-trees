# =============================================================================
# 06_mdf_lod.R  (BCI)
# Minimum Detectable Flux (MDF) / Limit of Detection (LOD) analysis. Identical
# methodology to transect-trees-2026/scripts/06_mdf_lod.R (three MDF definitions
# x 99/95/90% confidence). Only the compiled file names differ (bci_2026_*).
...
# --- Manufacturer 1-sigma precision (LI-7810) --------------------------------
...
compiled_path <- file.path(results_dir, "bci_2026_fluxes.csv")
if (file.exists(compiled_path)) {
  ...
  out <- left_join(compiled, mdf %>% select(-CO2_best.flux, -CH4_best.flux,
                   ...
                   by = "UniqueID")
  write.csv(out, file.path(results_dir, "bci_2026_fluxes_with_mdf.csv"),
            row.names = FALSE)
  write.xlsx(out, file.path(results_dir, "bci_2026_fluxes_with_mdf.xlsx"))
  message("\nSaved: ", file.path(results_dir, "bci_2026_fluxes_with_mdf.csv"))
} else {
