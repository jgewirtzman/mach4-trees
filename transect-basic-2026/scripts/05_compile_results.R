# =============================================================================
# 05_compile_results.R  (BASIC trees)
# Merge CO2 & CH4 goFlux results with field metadata. Identical to
# transect-trees-2026/05 except output names (transect_basic_2026_fluxes.*).
# Run with Rscript.
# =============================================================================
...
pick <- function(df, gas) {
  df %>% mutate(UniqueID = trimws(UniqueID)) %>%
    select(UniqueID, best.flux, model, quality.check,
           LM.flux, LM.r2, LM.p.val, LM.RMSE, HM.flux, HM.r2, HM.RMSE,
           LM.diagnose, HM.diagnose, nb.obs, flux.term, MDF, g.fact) %>%
...
final <- field_meta %>%
  ...
  arrange(TreeID, height_cm) %>%
  mutate(Area_cm2 = chamber_area_cm2, Vtot_L = Vtot_L,
         Tcham_C = Tcham_default_C, Pcham_kPa = Pcham_default_kPa)

write.csv(final, file.path(results_dir, "transect_basic_2026_fluxes.csv"), row.names = FALSE)
write.xlsx(final, file.path(results_dir, "transect_basic_2026_fluxes.xlsx"))
...
message("Saved transect_basic_2026_fluxes.csv. Proceed to 06_mdf_lod.R")
