# =============================================================================
# 05_compile_results.R
# Merge CO2 & CH4 goFlux results with field metadata into one tidy table.
# Carries through goFlux's MDF and g.fact columns. Mirrors whole_tree_flux
...
final <- field_meta %>%
  ...
  # record the chamber/T/P inputs that were used (set in 00_setup.R)
  mutate(Area_cm2 = chamber_area_cm2, Vtot_L = Vtot_L,
         Tcham_C = Tcham_default_C, Pcham_kPa = Pcham_default_kPa)

write.csv(final, file.path(results_dir, "transect_trees_2026_fluxes.csv"),
          row.names = FALSE)
write.xlsx(final, file.path(results_dir, "transect_trees_2026_fluxes.xlsx"))
...
message("Saved: ", file.path(results_dir, "transect_trees_2026_fluxes.csv"))
